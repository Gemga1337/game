...
function createCollectable(x, y, size, c1, c2) {
    return {
        ...
        color1: c1,
        color2: c2,
        moveRange: random(50, 150),     // Случайный диапазон движения
        moveSpeed: random(0.5, 2),     // Случайная скорость
        startX: x - random(0, 50),    // Начальная точка X
        startY: y - random(0, 30),    // Начальная точка Y
        directionX: random([-1, 1]),  // Начальное направление X
        directionY: random([-1, 1]),  // Начальное направление Y
        isMoving: true,              // Флаг движения
        ...
        update: function() {
            ...
            if (this.x > this.startX + this.moveRange || this.x < this.startX - this.moveRange) {
                this.directionX *= -1; // Меняем направление
            }
            
            // Движение по Y
            this.y += this.moveSpeed * this.directionY;
            if (this.y > this.startY + this.moveRange || this.y < this.startY - this.moveRange) {
                this.directionY *= -1; // Меняем направление
            }
        }
    };
}

// Остальные фабрики объектов (без изменений)
